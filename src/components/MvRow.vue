...
interface MvItemsType {
  ...
  cover?: string;
  coverUrl?: string;
  creator?: { userName: string; userId: number }[];
  artistId?: number;
  publishTime?: string;
}
...
const props = withDefaults(
  defineProps<{
    mvs: Omit<MvItemsType, "creator" | "artistId">[] | MvItemsType[];
    subtitle: string;
...
<template>
  <div class="mv-row" :class="{ 'without-padding': withoutPadding }">
    <div v-for="mv in mvs" :key="getID(mv)" class="mv">
      <div class="cover" @mouseover="hoverVideoID = getID(mv)" @mouseleave="hoverVideoID = 0" @click="goToMv(getID(mv))">
        <img alt="mv" :src="getUrl(mv)" loading="lazy" />
        <transition name="fade">
          <div v-show="hoverVideoID === getID(mv)" class="shadow" :style="{ background: 'url(' + getUrl(mv) + ')' }"></div>
        </transition>
      </div>
      <div class="info">
        <div class="title">
