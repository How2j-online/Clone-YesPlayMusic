...
const { lyricsSetting } = storeToRefs(settingStore);
...
const lang = computed({
  ...
  set: value => {
    settingStore.changeLang(value);
    changeLocale(value);
  }
});
const appearance = computed({
  get: () => {
    return settingStore.appearance;
  },
  set: value => {
    settingStore.changeAppearance(value);
  }
});

const musicLanguage = ref("all");
const musicQuality = ref("128000");
const showLyricsTranslation = computed({
  get: () => lyricsSetting.value.showLyricsTranslation,
  set: value => {
    settingStore.setLyricsSetting({ showLyricsTranslation: value });
  }
});
const lyricsBackground = computed({
  get: () => lyricsSetting.value.lyricsBackground,
  set: value => {
    console.log(value);
    settingStore.setLyricsSetting({ lyricsBackground: value });
  }
});
const showLyricsTime = ref(false);
const lyricFontSize = ref("16");
const showPlaylistsByAppleMusic = ref(false);
const subTitleDefault = ref(false);
const enableReversedMode = ref(false);
const version = ref("1.0.0");

const lastfm = ref({ name: "" });
const isLastfmConnected = ref(false);
  ...
  <div class="settings-page" @click="clickOutside">
    <div class="container">
      <div v-if="showUserInfo" class="user">
        <div class="left">
          <img class="avatar" :src="data.user.avatarUrl" loading="lazy" alt="img" />
          <div class="info">
        ...
        <div class="right">
          <button @click="logout">
            <svg-icon name="logout" class="svg-icon" />
            {{ t("settings.logout") }}
          </button>
        </div>
      </div>
      ...
      <div class="item">
        ...
        <div class="right">
          <div class="toggle">
            <input id="show-lyrics-translation" v-model="showLyricsTranslation" type="checkbox" name="show-lyrics-translation" />
      ...
      <div class="footer">
        <p class="author">MADE BY <a href="http://github.com/qier222" target="_blank">QIER222</a></p>
        <p class="version">v{{ version }}</p>

        <!--<a href="https://vercel.com/?utm_source=ohmusic&utm_campaign=oss">-->
        <!--  <img alt="img" height="36" src="https://www.datocms-assets.com/31049/1618983297-powered-by-vercel.svg" />-->
        <!--</a>-->
      </div>
    </div>
  </div>
...
.toggle input + label:after {
  ...
  position: absolute;
  display: block;
  box-shadow: 0 0 0 1px hsla(0, 0%, 0%, 0.02), 0 4px 0 0 hsla(0, 0%, 0%, 0.01), 0 4px 9px hsla(0, 0%, 0%, 0.08),
